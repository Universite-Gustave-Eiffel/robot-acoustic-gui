...
from PySide6.QtWidgets import (
    ...
    QHeaderView, QTableWidgetItem, QPushButton, QFileDialog
)
from PySide6.QtGui import QIcon, QFont, QAction, QCloseEvent, QColor
...
class TelecommandeWindow(QMainWindow):
    ...
    def _create_relative_control_panel(self) -> QWidget:
        ...
        for i, axis in enumerate(axes):
            ...
            btn_minus.clicked.connect(lambda checked=False, ax=axis, sign=-1: self.controller.move_robot_relative(ax,
                                                                                                                  sign *
                                                                                                                  self.spin_boxes[
                                                                                                                      ax].value()))
            spin_box = QSpinBox()
            ...
            btn_plus.clicked.connect(lambda checked=False, ax=axis, sign=1: self.controller.move_robot_relative(ax,
                                                                                                                sign *
                                                                                                                self.spin_boxes[
                                                                                                                    ax].value()))
    ...
    def update_position_display(self, positions: dict):
        self.pos_robot_x.setText(f"{positions.get('X', 0):.2f}")
        self.pos_robot_y.setText(f"{positions.get('Y', 0):.2f}")
        self.pos_robot_z.setText(f"{positions.get('Z', 0):.2f}")
        self.pos_robot_theta.setText(f"{positions.get('THETA', 0):.2f}")
        self.pos_robot_phi.setText(f"{positions.get('PHI', 0):.2f}")
...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self) -> None:
        ...
        self.controller.sequence_status_changed.connect(self.update_status_bar)  # Le statut de la séquence s'affiche aussi dans la barre d'état
    ...
    def _create_actions_and_connections(self) -> None:
        ...
        self._add_action('add_point', 'add.png', "Ajouter point", "Ajouter un nouveau point à la fin",
                         slot=self.controller.add_new_point)
        ...
        self._add_action('start_sequence', 'play.png', 'Démarrer séquence', "Démarrer la séquence de mesure",
                         slot=self.controller.start_sequence)
    ...
    def _on_save_triggered(self):
        if not self.controller.save_point_list(): self._on_save_as_triggered()
    ...
    def _on_save_as_triggered(self):
        ...
        if file_path: self.controller.save_point_list_to_file(file_path)
    ...
    def update_points_table(self, points: list):
        ...
        self.points_table.setHorizontalHeaderLabels([h.upper() for h in headers])
    ...
    def _on_delete_points_triggered(self):
        ...
        if reply == QMessageBox.StandardButton.Yes: self.controller.delete_selected_points(list(selected_rows))

    @Slot()
    def _on_move_up_triggered(self):
        selected_rows = {item.row() for item in self.points_table.selectedItems()}
        if len(selected_rows) == 1: self.controller.move_selected_point_up(list(selected_rows)[0])

    @Slot()
    def _on_move_down_triggered(self):
        selected_rows = {item.row() for item in self.points_table.selectedItems()}
        if len(selected_rows) == 1: self.controller.move_selected_point_down(list(selected_rows)[0])
    ...
    def highlight_table_row(self, row_index: int):
        """Surligne une ligne spécifique dans la table et réinitialise les autres."""
        ...
        if self.current_highlighted_row != -1 and self.current_highlighted_row < self.points_table.rowCount():
            for col in range(self.points_table.columnCount()):
                ...
                if item:
                    item.setBackground(QColor("white"))  # ou la couleur par défaut de ton thème
        ...
        if row_index != -1 and row_index < self.points_table.rowCount():
            for col in range(self.points_table.columnCount()):
                ...
                if item:
                    item.setBackground(QColor("#a8d8ea"))  # Une couleur bleu clair, par exemple
        ...
        if self.controller.is_modified:
            ...
            if reply == QMessageBox.StandardButton.Save:
                if not self.controller.save_point_list():
                    ...
                    if self.controller.is_modified: event.ignore(); return
